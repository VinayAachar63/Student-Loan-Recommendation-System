...
app.config['MAIL_PASSWORD'] = 'dyvl znqz pzgq kreq'  # App password
...
def chat():
    ...
    if not message:
        return jsonify({"response": "Please type a message.", "voice": None})
    ...
    return jsonify({"response": response_text, "voice": voice_base64})
...
def register():
    ...
    if not validate_name(name):
        return jsonify({"error": "Invalid name (only letters allowed)."}), 400
...
def recommend():
    ...
    if not validate_phone(data.get('phone', '')):
        return jsonify({"error": "Invalid phone. Only digits allowed."}), 400
    if not validate_aadhaar(data.get('aadhaar', '')):
        return jsonify({"error": "Invalid Aadhaar. Must be 12 digits."}), 400
    ...
    try:
        ...
        return jsonify({"error": "Numeric input error"}), 400
...
def apply():
    ...
    try:
        ...
        return jsonify({"message": "Application submitted & detailed email sent."})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
